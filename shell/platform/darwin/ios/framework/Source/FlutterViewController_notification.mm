...
- (void)handleKeyboardNotification:(NSNotification*)notification {
  ...
  if (self.targetViewInsetBottom == calculatedInset) {
    FML_LOG(INFO) << "Calculated inset: " << calculatedInset / 3;
    return;
  }

  FML_LOG(INFO) << "Update inset: " << calculatedInset / 3;
  // calculatedInset = calculatedInset + 50;
...
- (CGFloat)calculateKeyboardInsetWithScreen:(NSNotification*)notification {
  ...
  CGRect keyboardFrameEnd = [info[UIKeyboardFrameEndUserInfoKey] CGRectValue];

  // let fromCoordinateSpace: UICoordinateSpace = screen.coordinateSpace
  // UICoordinateSpace* fromCoordinateSpace = screen.coordinateSpace;
  ...
  CGFloat bottomOffset = self.viewIfLoaded.safeAreaInsets.bottom;
  ...
  CGRect intersection = CGRectIntersection(self.viewIfLoaded.bounds, convertedKeyboardFrameEnd);
  ...
  if (!CGRectIsEmpty(intersection)) {
    bottomOffset = CGRectGetHeight(intersection);
  }
...
- (CGFloat)calculateKeyboardInset:(CGRect)keyboardFrame keyboardMode:(NSInteger)keyboardMode {
  ...
  if (keyboardMode == FlutterKeyboardModeDocked) {
    ...
    // FML_LOG(INFO) << "Portion of keyboard in view: " << portionOfKeyboardInView;
