...
- (void)viewDidLoad {
  ...
  self.view.keyboardLayoutGuide.followsUndockedKeyboard = NO;
  ...
  if ([self keyboardAnimationView] == nil) {
    ...
    keyboardAnimationView.accessibilityIdentifier = @"Keyboard Inset UIView";
    // keyboardAnimationView.backgroundColor = [UIColor redColor];
...
- (void)handleKeyboardNotification:(NSNotification*)notification {
  ...
  // FML_LOG(INFO) << "New Inset: " << keyboardHeight * scale;

  // self.targetViewInsetBottom = keyboardHeight * scale; // Expect: 1008
...
- (void)updateTargetViewInsetBottom:(CGFloat)inset {
  self.targetViewInsetBottom = inset;
}
...
- (void)startKeyBoardAnimationWithKeyboardLayoutGuide {
  // If vsync is already in progress, don't start a new one.
  if (_keyboardAnimationVSyncClient != nil) {
    return;
  }

  FML_LOG(INFO) << "Start VSYNC";
  [self setupKeyboardAnimationVsyncClientWithKeyboardLayoutGuide];
}
  ...
  auto callback = [weakSelf =
    ...
    if ([flutterViewController keyboardAnimationView].layer.presentationLayer) {
      ...
      if ([flutterViewController targetViewInsetBottom] != targetKeyboardHeight) {
        FML_LOG(INFO) << "Update target inset";
        [flutterViewController updateTargetViewInsetBottom:targetKeyboardHeight];
      }
      if (currentKeyboardHeight == targetKeyboardHeight) {
        FML_LOG(INFO) << "Reached Target: " << targetKeyboardHeight;
...
- (void)startKeyBoardAnimation:(NSTimeInterval)duration {
  ...
  if ([self keyboardAnimationView] == nil) {
    UIView* keyboardAnimationView = [[UIView alloc] init];
    keyboardAnimationView.backgroundColor = [UIColor redColor];
    [keyboardAnimationView setHidden:NO];
    _keyboardAnimationView.reset(keyboardAnimationView);
  }
